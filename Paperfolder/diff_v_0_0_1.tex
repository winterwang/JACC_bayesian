...
%DIF ADD Paperfolder.tex          Thu Jul 16 17:48:17 2020
...
\DIFaddbegin \DIFadd{Eastern Asian populations were reported to have higher burden from
either mortality or morbidity from stroke than populations in European
or American regions \mbox{%DIFAUXCMD
\citep{Kim2014}}\hspace{0pt}%DIFAUXCMD
. }\DIFaddend Dairy food, especially milk has
been \DIFdelbegin \DIFdel{recommended }\DIFdelend \DIFaddbegin \DIFadd{suggested }\DIFaddend to reduce stroke risk by nearly 7\% for each 200 g
increment of daily consumption \citep{DeGoede2016}. \DIFdelbegin \DIFdel{More intuitive interpretation for a decreasing risk
...
public}\DIFdelend \DIFaddbegin \DIFadd{Although 2 daily
servings of milk or dairy products is recommended in Japan
\mbox{%DIFAUXCMD
\citep{Yoshiike2007}}\hspace{0pt}%DIFAUXCMD
, the actual per capita intake (\(\approx\) 63
g}\DIFaddend /\DIFdelbegin \DIFdel{media reporting, concept of hazard in epidemiological studies
...
an event at a given time point \mbox{%DIFAUXCMD
\citep{collett2015modelling}}\hspace{0pt}%DIFAUXCMD
. Usually,
...
For better understanding and interpretation of
the findings from data that researchers endeavored to collect, statistical literature have provided plenty of choices that could help
us better communicate with each other. Another approach of comparing the
time-to-event survival probabilities between different groups would be to model the time before observing an event rather than the hazard which
always required the assumption of a proportional hazard to be met}\DIFdelend \DIFaddbegin \DIFadd{day) of these food groups is much lower and less frequent than that in
the Western countries \mbox{%DIFAUXCMD
\citep{Saito2019}}\hspace{0pt}%DIFAUXCMD
. Given that previous reports
have also indicated no significant
\mbox{%DIFAUXCMD
\citep{Iso1999, sauvaget2003intake, Elwood2004} }\hspace{0pt}%DIFAUXCMD
or even positive
associations \mbox{%DIFAUXCMD
\citep{Larsson2009}}\hspace{0pt}%DIFAUXCMD
, whether people with such a low level
intake of milk can still benefit against stroke would be questionable.
Even if there is any benefit, not every individual with habitual milk
intake can avoid dying from stroke. A more intuitive interpretation
would be available if we were able to show the probabilities and how
certain the existing data can provide evidence about whether drinking
milk can delay a stroke event or lower the hazard of dying from stroke}\DIFaddend .
Accelerated failure time models \DIFdelbegin \DIFdel{are among these }\DIFdelend \DIFaddbegin \DIFadd{under Bayesian framework are }\DIFaddend convenient
tools that would \DIFaddbegin \DIFadd{help }\DIFaddend avoid worrying about the assumption of
proportional hazard \DIFdelbegin \DIFdel{and
directly showing }\DIFdelend \DIFaddbegin \DIFadd{normally required in a Cox proportional hazard model
and can directly show }\DIFaddend how faster/slower \DIFdelbegin \DIFdel{one individual }\DIFdelend \DIFaddbegin \DIFadd{individuals }\DIFaddend in an exposure group
might have an event compared to others among different exposure groups
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Wei1992}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Wei1992, ibrahim2014b}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .

Our aim was to \DIFdelbegin \DIFdel{overcome these potential pitfalls, avoid
misunderstanding, and }\DIFdelend provide a more straightforward answer to the \DIFdelbegin \DIFdel{main
}\DIFdelend \DIFaddbegin \DIFadd{primary
}\DIFaddend research question that whether someone answered he/she drank milk at the
...
provided estimates about whether drinking milk could delay \DIFdelbegin \DIFdel{or slow down
the speed towards a mortality from stroke
}\DIFdelend \DIFaddbegin \DIFadd{a stroke
mortality }\DIFaddend event from happening after controlling for the other potential
confounders.
...
intake. \DIFdelbegin \DIFdel{Overall difference across the milk intake groups were tested by
either analysis of variance for continuous variables or \(\chi^2\) test
for categorical variables}\DIFdelend \DIFaddbegin \DIFadd{Age-adjusted stroke mortality rate were expressed as per 1000
person-year through the poisson regression model}\DIFaddend .
...
women). \DIFaddbegin \DIFadd{Age-adjusted stroke mortality rate for each category of milk
...
Some limitations here are worth mentioning. First, \DIFdelbegin \DIFdel{although our object
...
detailed comparison or stratification would have been possible}\DIFdelend \DIFaddbegin \DIFadd{the milk intake
frequency as well as other lifestyle information was collected only once
at the baseline and was self-reported. Apparently habits in life are
possible to alter over the time and these would resulted in
misclassfication and residual confoudings}\DIFaddend . Second, despite reasonable
validity of FFQ in the JACC study cohort was assessed and confirmed,
measurement errors are inevitable. Therefore, we did not try to compute
the amount of consumption by multiplying an average volume per occasion
with the frequency of intake since the random error might be exaggerated
and the observed associations may have attenuated. Strengths of our
analyses included that we have transformed the research questions to
